...
impl ServoEngine {
    ...
    ///    walks the page top→bottom in **two passes of 6 steps each**
    ///    (12 total scroll positions, ~3.5s) with a longer pause at
    ///    the end of each pass. At every step the script both calls
    ///    `window.scrollTo()` *and* dispatches a synthetic `WheelEvent`
    ///    to both `window` and `document` — many lazy-loaders gate on
    ///    `'wheel'` rather than `'scroll'`, and `scrollTo()` does not
    ///    fire wheel events. The two passes catch the chained case
    ///    where the first pass reveals lazy-loaded content whose own
    ///    IntersectionObservers need a second pass to fire; the
    ///    pass-end pause gives any chained `setTimeout(0)` / fetch
    ///    work time to land. Without this, GitHub topic pages and
    ///    similar scroll-triggered lists reach a stable DOM almost
    ///    immediately because the repo cards are gated behind an
    ///    IntersectionObserver that never observes anything below
    ///    the fold until the user scrolls. The `grace_ms` floor on
    ///    top of `idle_ms` covers the asymmetric case where a
    ///    lazy-load schedules work but its first mutation lands
    ///    slightly after we declare stability. If the heuristic
    ///    still misses a site, the `POST /native-input` endpoint on
    ///    `browsai serve` exposes raw `NativeInputEvent` dispatch so
    ///    a plugin can run its own scroll-and-snapshot loop with
