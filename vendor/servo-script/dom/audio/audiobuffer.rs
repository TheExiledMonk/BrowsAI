...
impl AudioBufferMethods<crate::DomTypeHolder> for AudioBuffer {
    ...
    fn CopyFromChannel(
        ...
        {
            // `dest` is already sized to the number of samples requested.
            // Appending here doubled its length and made the subsequent
            // TypedArray update violate the JS destination buffer size.
            dest.copy_from_slice(&shared_channel.as_slice()[offset..offset + bytes_to_copy]);
        }
