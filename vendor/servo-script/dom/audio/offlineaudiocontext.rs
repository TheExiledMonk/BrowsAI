...
impl OfflineAudioContextMethods<crate::DomTypeHolder> for OfflineAudioContext {
    ...
    fn StartRendering(&self, cx: &mut CurrentRealm) -> Rc<Promise> {
        ...
        // An offline context is running while its render quantum is being
        // processed. Leaving the public state suspended until completion
        // causes feature-detection code to call startRendering() again,
        // which correctly rejects the second call with InvalidStateError.
        if self.context.State() != AudioContextState::Running {
            self.context.set_state_attribute(AudioContextState::Running);
            self.global()
                .task_manager()
                .dom_manipulation_task_source()
                .queue_simple_event(self.upcast(), atom!("statechange"));
        }
