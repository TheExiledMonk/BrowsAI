...
fn console_argument_from_handle_value(
    ...
    fn inner(
        ...
        if handle_value.is_object() {
            ...
            // Property enumeration can re-enter the printer through hostile
            // or proxy-backed objects. Do not turn an imbalanced diagnostic
            // stack into a script-thread crash; remove this frame and any
            // entries left by the re-entrant inspection.
            if let Some(index) = seen.iter().rposition(|bits| *bits == handle_value.asBits_) {
                seen.truncate(index);
            } else {
                seen.clear();
            }
