    ...
    pub(crate) fn notify_font_face_rules_removed(
        ...
        for removed_font_face_rule in removed_font_face_rules {
            let Some(matching_font_face_object) = entries
                ...
                // Stylesheet replacement and duplicate mutation notifications
                // can report the same rule more than once. Removal is
                // idempotent from the FontFaceSet's perspective; do not bring
                // down the script thread for a stale callback.
                log::warn!("Removed @font-face was not present in FontFaceSet");
                continue;
            };
