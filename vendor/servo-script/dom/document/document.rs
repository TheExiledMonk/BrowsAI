...
impl Document {
    ...
    pub(crate) fn pending_parsing_blocking_script_loaded(
    ...
    ) {
        {
            ...
            let Some(entry) = blocking_script.as_mut() else {
                // The parser may have been torn down before the fetch
                // completion arrived. There is no pending script to resume.
                return;
            };
            if &*entry.element != element {
                // Ignore a late completion for an older element rather than
                // corrupting the current parser state.
                return;
            }
            entry.loaded(result);
        }
    ...
    fn process_pending_parsing_blocking_script(&self, cx: &mut JSContext) {
        ...
        if let Some((element, result)) = pair {
            *self.pending_parsing_blocking_script.borrow_mut() = None;
            let Some(parser) = self.get_current_parser() else {
                // The parser may have been detached while an async blocking
                // script completed.  There is no document parser to resume;
                // dropping the completion is safer than aborting the script
                // thread.
                return;
            };
            parser.resume_with_pending_parsing_blocking_script(cx, &element, result);
        }
    }
    ...
    pub(crate) fn asap_script_loaded(
    ...
    ) {
        {
            ...
            let Some(idx) = scripts.iter().position(|entry| &**entry == element) else {
                // A document teardown or duplicate completion can remove the
                // entry before the asynchronous fetch callback arrives.
                // Treat the callback as stale instead of aborting the script
                // thread.
                return;
            };
...
impl Document {
    ...
    fn new_with_proto(
        ...
        let diagnostic_status_code = status_code;
        ...
        if std::env::var_os("BROWSAI_DIAGNOSTIC_STATUS").is_some() {
            if let Some(status_code) = diagnostic_status_code {
            rooted!(&in(cx) let mut value = js::jsval::UndefinedValue());
            status_code.safe_to_jsval(cx, value.handle_mut());
            #[expect(unsafe_code)]
            unsafe {
                JS_DefineProperty(
                    cx,
                    document.reflector().get_jsobject(),
                    c"__browsaiStatusCode".as_ptr(),
                    value.handle(),
                    0,
                );
            }
    ...
    pub(crate) fn performance_timing_attribute(
        ...
        Ok(match name {
            ...
            // The legacy PerformanceTiming interface is still used by a
            // number of production bundles. Keep these aliases backed by the
            // same fetch timing record used by PerformanceNavigationTiming;
            // returning an implementation error here makes otherwise valid
            // pages fail during their startup telemetry probes.
            "fetchStart" | "domainLookupStart" | "domainLookupEnd" | "connectStart"
            | "connectEnd" | "requestStart" | "responseStart" => self
                .resource_fetch_timing()
                .as_ref()
                .and_then(|resource_fetch_timing| match name {
                    "fetchStart" => resource_fetch_timing.fetch_start,
                    "domainLookupStart" => resource_fetch_timing.domain_lookup_start,
                    // The network timing record does not currently retain a
                    // separate lookup-end instant; leave this milestone
                    // unset rather than reporting an implementation error.
                    "domainLookupEnd" => None,
                    "connectStart" => resource_fetch_timing.connect_start,
                    "connectEnd" => resource_fetch_timing.connect_end,
                    "requestStart" => resource_fetch_timing.request_start,
                    "responseStart" => resource_fetch_timing.response_start,
                    _ => None,
                }),
            // Servo does not currently expose a separate DOM-loading instant.
            // Fetch start is the closest available navigation milestone and
            // is preferable to throwing for the legacy alias.
            "domLoading" => self
                .resource_fetch_timing()
                .as_ref()
                .and_then(|resource_fetch_timing| resource_fetch_timing.fetch_start),
            _ => {
