...
use crate::dom::node::{
    ...
    NodeTraits, UnbindContext, simple_has_selector_groups,
};
...
impl ElementMethods<crate::DomTypeHolder> for Element {
    ...
    fn GetBoundingClientRect(&self, cx: &mut JSContext) -> DomRoot<DOMRect> {
        ...
        // Layout can transiently expose a negative intermediate size while a
        // responsive subtree is being updated. CSSOM View exposes a DOMRect;
        // keep the public dimensions valid instead of asserting in the script
        // thread during that transient state.
        let width = rect.size.width.to_f64_px().max(0.0);
        let height = rect.size.height.to_f64_px().max(0.0);
        DOMRect::new(
            cx,
            win.upcast(),
            rect.origin.x.to_f64_px(),
    ...
    fn Matches(&self, selectors: DOMString) -> Fallible<bool> {
        if let Some(groups) = simple_has_selector_groups(&selectors.str()) {
            for (base, descendant, suffix) in groups {
                if !suffix.is_empty() {
                    if !self.Matches(DOMString::from(suffix))? {
                        continue;
                    }
                    for node in self
                        .upcast::<Node>()
                        ...
                        .skip(1)
                    {
                        ...
                        if !ancestor.Matches(DOMString::from(base.clone()))? {
                            continue;
                        }
                        let Some(descendant) = descendant.as_ref() else {
                            return Ok(true);
                        };
                        let mut has_descendant = false;
                        for descendant_node in ancestor
                            .upcast::<Node>()
                            .traverse_preorder(ShadowIncluding::No)
                            .skip(1)
                        {
                            let Some(descendant_element) =
                                descendant_node.downcast::<Element>()
                            else {
                                continue;
                            };
                            if descendant_element
                                .Matches(DOMString::from(descendant.clone()))?
                            {
                                has_descendant = true;
                                break;
                            }
                ...
                if !self.Matches(DOMString::from(base))? {
                    continue;
                }
                    ...
                    .skip(1)
                {
                    ...
                    if element.Matches(DOMString::from(descendant.clone()))? {
                        return Ok(true);
                    }
                }
            }
