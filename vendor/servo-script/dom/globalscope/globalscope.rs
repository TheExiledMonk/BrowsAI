...
use net_traits::image_cache::{
    ImageCache, ImageCacheResponseCallback, ImageCacheResult, ImageLoadListener, PendingImageId,
};
...
struct ServiceWorkerImageCache;

fn service_worker_image_cache() -> Arc<dyn ImageCache> {
    static CACHE: OnceLock<Arc<ServiceWorkerImageCache>> = OnceLock::new();
    CACHE
        .get_or_init(|| Arc::new(ServiceWorkerImageCache))
        .clone()
}

impl ImageCache for ServiceWorkerImageCache {
    ...
    fn add_rasterization_complete_listener(
        &self,
        _pipeline_id: servo_base::id::PipelineId,
        _image_id: PendingImageId,
        _size: DeviceIntSize,
    ...
    fn notify_pending_response(
        &self,
        _id: PendingImageId,
        _action: net_traits::FetchResponseMsg,
    ) {
    }
...
impl GlobalScope {
    ...
    pub(crate) fn image_cache(&self) -> Arc<dyn ImageCache> {
        ...
        // ServiceWorkerGlobalScope does not have a document/pipeline image cache.
        // Keep capability probes and constructors deterministic instead of reaching
        // an unreachable branch when an API such as OffscreenCanvas asks for one.
        if self.downcast::<ServiceWorkerGlobalScope>().is_some() {
            return service_worker_image_cache();
        }
        unreachable!();
    }
    ...
    pub(crate) fn evaluate_js_on_global(
        ...
        if !self.can_run_script() {
            // A navigation or teardown can race an embedder evaluation
            // request. Match the other script entry points and return a
            // recoverable readiness error instead of asserting on the script
            // thread.
