...
use crate::indexeddb::{
    convert_value_to_key, extract_key, extract_multi_entry_keys, key_type_to_jsval,
    ExtractionResult,
};
...
impl IDBCursor {
    pub(crate) fn index_records(
        ...
        let ObjectStoreOrIndex::Index(index) = &self.source else {
            return Ok(records);
        };
        let key_path = index.cursor_key_path();
        let multi_entry = index.cursor_multi_entry();
        ...
        for record in records {
            ...
            if multi_entry {
                if let Some(keys) = extract_multi_entry_keys(cx, value.handle(), &key_path)? {
                    for key in keys {
                        indexed.push(IndexedDBRecord {
                            key,
                ...
                match extract_key(cx, value.handle(), &key_path, None)? {
                    ExtractionResult::Key(key) => indexed.push(IndexedDBRecord {
                        key,
...
impl IDBCursorMethods<crate::DomTypeHolder> for IDBCursor {
    ...
    /// Re-run the cursor's existing request with the cursor position advanced.
    ///
    /// Servo's IDL previously omitted these methods entirely.  The request and
    /// backend iteration machinery already exists, so reusing the request here
    /// keeps cursor success events and `result` identity intact.
    #[allow(unsafe_code)]
    fn Advance(&self, count: u32) -> Result<(), Error> {
        if count == 0 {
            return Err(Error::InvalidState(Some(
                "Cursor advance count must be greater than zero".into(),
            )));
        }
        let mut cx = unsafe { temp_cx() };
        self.restart_iteration(&mut cx, None, None, Some(count))
    }

    #[allow(unsafe_code)]
    fn Continue(&self, key: HandleValue) -> Result<(), Error> {
        let mut cx = unsafe { temp_cx() };
        ...
        self.restart_iteration(&mut cx, key, None, None)
    }
    ...
    fn ContinuePrimaryKey(
        ...
        // IDBIndex currently falls back to the underlying object-store cursor
        // in the compatibility layer.  In that representation the supplied
        // primary key is the cursor key, so preserving it gives callers the
        // expected forward/reverse continuation behavior without pretending
        // that index-key ordering is available.
        #[allow(unsafe_code)]
        let mut cx = unsafe { temp_cx() };
        let key = convert_value_to_key(&mut cx, _key, None)?.into_result()?;
        let primary_key = convert_value_to_key(&mut cx, primary_key, None)?.into_result()?;
        self.restart_iteration(&mut cx, Some(key), Some(primary_key), None)
    }
}

impl IDBCursor {
    fn restart_iteration(
        &self,
        cx: &mut JSContext,
        ...
        let request = self
            .request
            .get()
            .ok_or_else(|| Error::InvalidState(Some("Cursor has no request".into())))?;
        let source = match &self.source {
            ObjectStoreOrIndex::ObjectStore(source) => source.as_rooted(),
            ObjectStoreOrIndex::Index(source) => source.cursor_object_store(),
        };
