...
impl IDBDatabase {
    ...
    pub(crate) fn clear_upgrade_transaction(&self, transaction: &IDBTransaction) {
        // Completion and abort paths can both be queued for the same version
        // change transaction. Treat a stale cleanup callback as idempotent
        // instead of panicking the script thread after the transaction slot
        // has already been cleared. Never clear a newer upgrade transaction
        // that replaced the stale callback's transaction.
...
impl IDBDatabaseMethods<crate::DomTypeHolder> for IDBDatabase {
    ...
    fn Transaction(
        ...
        let transaction =
            IDBTransaction::new(cx, &self.global(), self, mode, durability, &scope)?;
