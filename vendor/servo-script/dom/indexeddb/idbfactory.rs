...
use storage_traits::indexeddb::{
    BackendError, BackendResult, ConnectionMsg, DatabaseInfo, IndexedDBThreadMsg, SyncOperation,
};
...
use crate::dom::bindings::error::{Error, ErrorInfo, ErrorToJsval, Fallible};
...
impl IDBFactory {
    ...
    pub(crate) fn clear_open_request_transaction_for_txn(&self, transaction: &IDBTransaction) {
        ...
        // A completion callback can race with request cleanup or be delivered
        // more than once by a site that opens several versionchange requests.
        // The request-side clear operation is idempotent; do not abort the
        // script thread merely because a stale callback cleared zero entries
        // (or because an older duplicate callback cleared more than one).
        let _ = cleared;
    }
    ...
    fn handle_connection_message(&self, cx: &mut JSContext, response: ConnectionMsg) {
        match response {
            ...
            ConnectionMsg::VersionChange {
                ...
                let Some(connection) = request.connection() else {
                    return;
                };
    ...
    fn dispatch_error(
        ...
        error!("IndexedDB open failed for {:?}: {:?}", name, dom_exception);
        if std::env::var_os("BROWSAI_IDB_DIAGNOSTICS").is_some() {
            self.global().report_an_error(
                cx,
                ErrorInfo {
                    message: format!(
                        "BROWSAI_INDEXEDDB_OPEN_ERROR name={name:?} error={dom_exception:?}"
                    ),
                    ..Default::default()
                },
                HandleValue::null(),
...
impl IDBFactoryMethods<crate::DomTypeHolder> for IDBFactory {
    ...
    fn Databases(&self, cx: &mut JSContext) -> Rc<Promise> {
        ...
        let task_source = global
            ...
            .to_sendable();
        let callback = GenericCallback::new(global.time_profiler_chan().clone(), move |message| {
            let result: BackendResult<Vec<DatabaseInfo>> = message.unwrap_or_else(|_| {
                Err(BackendError::DbErr("IndexedDB channel disconnected".into()))
            });
