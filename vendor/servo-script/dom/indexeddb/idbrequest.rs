        ...
        if let Ok(data) = result {
            match data {
                ...
                IdbResult::Iterate(records) => {
                    let Some(param) = self.iteration_param.as_ref() else {
                        Self::handle_async_request_error(
                            &global,
                            cx,
                            request,
                            Error::Operation(Some("IndexedDB cursor metadata was lost".into())),
                            self.request_id,
                        );
                        return;
                    };
                    let records = match param.cursor.root().index_records(cx, &global, records) {
                        Ok(records) => records,
                        Err(e) => {
                            warn!("Error preparing IndexedDB index cursor records");
                            Self::handle_async_request_error(&global, cx, request, e, self.request_id);
                            return;
                        },
                    };
