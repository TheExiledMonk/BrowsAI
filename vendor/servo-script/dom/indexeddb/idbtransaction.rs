...
impl IDBTransaction {
    ...
    fn create_transaction(
        ...
        let scope: Vec<String> = (0..scope.Length())
            ...
            .map(String::from)
            .collect();
        let (sender, receiver) = channel(global.time_profiler_chan().clone())
        ...
        let result = receiver.recv().map_err(|_| Error::Operation(None))?;
        result.map_err(map_backend_error_to_dom_error)
    }
    ...
    fn object_store_parameters(
        ...
        let object_store = match receiver.recv() {
            Ok(Ok(object_store)) => object_store,
            Ok(Err(error)) => {
                // Keep the backend error visible when a connection advertises a store
                // that the storage engine cannot resolve.  This is otherwise surfaced
                // to sites only as a repeated, context-free NotFoundError.
                warn!(
                    "IndexedDB GetObjectStore failed: origin={:?} database={:?} store={:?} error={:?}",
                    origin,
                    db_name,
                    object_store_name,
                    error
            ...
            Err(error) => {
                warn!(
                    "IndexedDB GetObjectStore response failed: origin={:?} database={:?} store={:?} error={:?}",
                    origin,
                    db_name,
                    object_store_name,
                    error
