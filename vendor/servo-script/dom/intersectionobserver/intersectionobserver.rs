    ...
    pub(crate) fn update_intersection_observations_steps(
        ...
        for target in &*self.observation_targets.borrow() {
            ...
            let Some(registration) = target.get_intersection_observer_registration(self) else {
                // The target may have been detached and unregistered between
                // observation and this update task. Ignore that stale target.
                continue;
            };
        ...
        let containing_element = match container {
            ElementOrDocument::Document(ref containing_document) => {
                    ...
                    .and_then(|window| window.frame_element().map(DomRoot::from_ref))
                {
                    ...
                    // A frame can be detached between the browsing-context
                    // lookup and layout traversal. IntersectionObserver is a
                    // best-effort notification API; an unavailable padding
                    // box means there is no valid coordinate mapping, not an
                    // engine-fatal invariant violation.
                    let Some(padding_box) = frame_container.upcast::<Node>().padding_box() else {
                        return None;
                    };
                    let current_offset = padding_box.origin.to_vector();
