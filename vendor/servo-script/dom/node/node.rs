...
/// Return selector groups that can be evaluated without the relational
/// :has() pseudo-class. This is intentionally limited to simple, balanced
/// base:has(descendant) groups with an optional trailing descendant selector;
/// unsupported forms continue through the
/// native selector parser and report its normal error.
pub(crate) fn simple_has_selector_groups(
    selectors: &str,
) -> Option<Vec<(String, Option<String>, String)>> {
    if !selectors.contains(":has(") {
        return None;
    }
    ...
    for (index, character) in selectors.char_indices() {
        match character {
            ...
            ')' => depth -= 1,
            ',' if depth == 0 => {
                groups.push(&selectors[start..index]);
    ...
    groups.push(&selectors[start..]);

    let mut parsed = Vec::with_capacity(groups.len());
    for group in groups {
        let group = group.trim();
        if let Some(position) = group.find(":has(") {
            let close = group[position + 5..]
                .find(')')
                .map(|offset| position + 5 + offset)?;
            if group[position + 5..close].contains('(')
                || group[close + 1..].contains(":has(")
            {
                return None;
            }
            let base = group[..position].trim();
            let descendant = group[position + 5..close].trim();
            let suffix = group[close + 1..].trim();
            if base.is_empty() || descendant.is_empty() {
                return None;
            }
            parsed.push((
                base.to_owned(),
            ...
            parsed.push((group.to_owned(), None, String::new()));
        }
    }
    Some(parsed)
}
...
impl Node {
    ...
    pub(crate) fn query_selector(
        ...
        if let Some(groups) = simple_has_selector_groups(&selectors.str()) {
            let document_url = self.owner_document().url().get_arc();
            for (base, descendant, suffix) in groups {
                let traced_node = Dom::from_ref(self);
                let candidates = with_layout_state(|| {
                    let layout_node: LayoutDom<'_, _> =
                        unsafe { traced_node.to_layout() };
                    ServoDangerousStyleNode::from(layout_node)
                        .scope_match_a_selectors_string::<QueryAll>(
                            document_url.clone(),
                            &base,
                        )
                        .map(|elements| elements.into_iter().collect::<Vec<_>>())
                })?;
                for candidate in candidates {
                    let element = ServoDangerousStyleElement::rooted(candidate);
                    if let Some(descendant) = descendant.as_ref() {
                        if element
                            .upcast::<Node>()
                            .query_selector(no_gc, DOMString::from(descendant.clone()))?
                            .is_none()
                        {
                            continue;
                        }
                    ...
                    if !suffix.is_empty() {
                        return element
                            .upcast::<Node>()
                            .query_selector(no_gc, DOMString::from(suffix));
                    }
    ...
    pub(crate) fn query_selector_all(
        ...
        if let Some(groups) = simple_has_selector_groups(&selectors.str()) {
            ...
            for (base, descendant, suffix) in groups {
                let candidates = self.query_selector_all(cx, DOMString::from(base))?;
                let candidate_elements: Vec<DomRoot<Element>> = {
                    let no_gc = cx.no_gc();
                    candidates
                        .iter(no_gc)
                ...
                for element in candidate_elements {
                    if let Some(descendant) = descendant.as_ref() {
                        if element
                            ...
                            .query_selector(
                                cx.no_gc(),
                                DOMString::from(descendant.clone()),
                            )?
                            .is_none()
                        {
                            continue;
                    ...
                    if !suffix.is_empty() {
                        let suffix_matches = element
                            .upcast::<Node>()
                            .query_selector_all(cx, DOMString::from(suffix.clone()))?;
                        let no_gc = cx.no_gc();
                        for matched in suffix_matches.iter(no_gc) {
                            matches.push(matched.as_rooted());
                        }
                        continue;
                    }
                    matches.push(DomRoot::<Element>::upcast(element));
                }
            }
            return Ok(NodeList::new_simple_list(
                cx,
                &self.owner_window(),
                matches.into_iter(),
            ));
