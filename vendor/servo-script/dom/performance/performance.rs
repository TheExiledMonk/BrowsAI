...
impl PerformanceMethods<crate::DomTypeHolder> for Performance {
    ...
    fn Measure(
        ...
        // end, or duration exists, run the following checks. `detail` may be
        // supplied on its own; current Chrome accepts
        // `performance.measure(name, { detail })` and uses the default
        // start/end timestamps in that case.
