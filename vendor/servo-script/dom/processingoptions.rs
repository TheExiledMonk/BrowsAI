...
impl LinkProcessingOptions {
    ...
    pub(crate) fn create_link_request(self, webview_id: WebViewId) -> Option<RequestBuilder> {
        // A link can be observed between element creation and href/srcset
        // population.  The HTML algorithm normally avoids constructing a
        // request in that state; keep the script thread alive if a queued
        // preload reaches this path anyway.
