...
pub(crate) fn load_script(head: &HTMLHeadElement) {
    ...
    doc.add_delayed_task(task!(UserScriptExecute: |cx, win: DomRoot<Window>| {
        ...
        // A navigation can tear down the document after this delayed task is
        // queued but before it runs. Do not evaluate user content against a
        // dead global; doing so used to trip the assertion in
        // `evaluate_js_on_global` and strand the embedding event loop.
