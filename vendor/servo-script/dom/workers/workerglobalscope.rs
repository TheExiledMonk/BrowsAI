...
const CACHE_WORKER_COMPATIBILITY_SCRIPT: &str = r#"
(function(){
  if(typeof self==='undefined'||typeof self.Cache!=='undefined')return;
  function Cache(){}
  self.Cache=Cache;
})();
"#;
...
impl WorkerGlobalScope {
    ...
    pub(crate) fn on_complete(&self, cx: &mut JSContext, script: Option<Script>) {
        ...
        // User-content scripts are installed on document globals, not on
        // dedicated/shared worker globals. Install the small Cache interface
        // identity before page worker code runs so feature probes observe the
        // same standard surface in both realms.
        let compatibility_script = self.globalscope.create_a_classic_script(
            cx,
            Cow::Borrowed(CACHE_WORKER_COMPATIBILITY_SCRIPT),
            self.get_url().clone(),
            ScriptFetchOptions::default_classic_script(),
            ErrorReporting::Unmuted,
            Some(IntroductionType::WORKER),
        ...
        let _ = self
            .globalscope
            .run_a_classic_script(cx, compatibility_script, RethrowErrors::No);
