...
pub(crate) fn load_whole_resource(
    ...
    if core_resource_thread
        ...
        .is_err()
    {
        // The resource thread can disappear while a worker is being torn down.
        // Treat that lifecycle race as a normal network failure instead of
        // panicking the script thread and leaving the embedder waiting.
    ...
    loop {
        match action_receiver.recv() {
            Err(_) => return Err(NetworkError::ConnectionFailure),
            Ok(FetchResponseMsg::ProcessRequestBody(..)) => {},
            Ok(FetchResponseMsg::ProcessResponse(_, Ok(m))) => {
            ...
            Ok(FetchResponseMsg::ProcessResponseChunk(_, data)) => buf.extend_from_slice(&data),
            Ok(FetchResponseMsg::ProcessResponseEOF(_, Ok(_), _)) => {
            ...
            Ok(FetchResponseMsg::ProcessResponse(_, Err(e))) |
            Ok(FetchResponseMsg::ProcessResponseEOF(_, Err(e), _)) => return Err(e),
            Ok(FetchResponseMsg::ProcessCspViolations(_, violations)) => {
                csp_violations_processor.process_csp_violations(cx, violations);
            },
            Ok(FetchResponseMsg::ProcessContentLength(_request_id, size)) => {
