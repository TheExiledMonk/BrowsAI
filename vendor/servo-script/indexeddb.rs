...
/// Extract the individual keys used by a multiEntry index.  This is kept
/// separate from `extract_key`, whose scalar result is also used for inline
/// object-store keys.
#[expect(unsafe_code)]
pub(crate) fn extract_multi_entry_keys(
    cx: &mut JSContext,
    value: HandleValue,
    key_path: &KeyPath,
) -> Result<Option<Vec<IndexedDBKeyType>>, Error> {
    rooted!(&in(cx) let mut evaluated = UndefinedValue());
    if let EvaluationResult::Failure =
        evaluate_key_path_on_value(cx, value, key_path, evaluated.handle_mut())?
    {
        return Ok(None);
    }
    let mut is_array = false;
    if !unsafe { IsArrayObject(cx, evaluated.handle(), &mut is_array) } {
        return Err(Error::JSFailed);
    }
    if !is_array {
        return Ok(None);
    }
    rooted!(&in(cx) let object = evaluated.to_object());
    let mut length = 0;
    if !unsafe { GetArrayLength(cx, object.handle(), &mut length) } {
        return Err(Error::JSFailed);
    }
    let mut keys = Vec::with_capacity(length as usize);
    for index in 0..length {
        rooted!(&in(cx) let mut id: PropertyKey);
        if !unsafe { JS_IndexToId(cx, index, id.handle_mut()) } {
            return Err(Error::JSFailed);
        }
        let mut own = false;
        if !unsafe { JS_HasOwnPropertyById(cx, object.handle(), id.handle(), &mut own) } {
            return Err(Error::JSFailed);
        }
        if !own {
            continue;
        ...
        rooted!(&in(cx) let mut entry = UndefinedValue());
        if !unsafe { js::rust::wrappers2::JS_GetPropertyById(
            cx,
            object.handle(),
            id.handle(),
            entry.handle_mut(),
        ...
        if let ConversionResult::Valid(key) = convert_value_to_key(cx, entry.handle(), None)? {
