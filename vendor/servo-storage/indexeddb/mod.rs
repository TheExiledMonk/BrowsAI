...
impl<E: KvsEngine> IndexedDBEnvironment<E> {
    ...
    fn abort_transaction(&mut self, txn: u64) {
        ...
        let _ = self.restore_key_generators_after_abort(&key_generator_snapshots);
...
impl IndexedDBManager {
    ...
    fn handle_upgrade_transaction_finished(
        ...
        if committed {
            ...
            let Some(front) = queue.front() else {
                // A stale transaction-finished notification can arrive after
                // the open request was already pruned. Treat it as an
                // idempotent completion rather than panicking the storage
                // thread.
                return;
            };
        ...
        let (request_id, proxy_map) = {
            let Some(queue) = self.connection_queues.get_mut(&key) else {
                // Notifications can race with queue pruning when a page is
                // closed during an IndexedDB operation. Treat this as a
                // stale notification instead of aborting the manager.
                return;
            };
            let Some(front) = queue.front() else {
                // A stale transaction-finished notification can arrive after
                // the open request was already pruned. Treat it as an
                // idempotent abort rather than panicking the storage thread.
                return;
            };
            let OpenRequest::Open {
                pending_upgrade: Some(pending_upgrade),
    ...
    fn revert_aborted_upgrade(
        ...
        if db.set_version(upgrade.old).is_err() {
            return;
        }
        ...
        if db.restore_object_stores(&upgrade.object_stores).is_err() {
            return;
        }
    }
    ...
    fn handle_version_change_done(
        ...
        let (can_upgrade, version) = {
            ...
            // A duplicate close/versionchange callback can arrive after the
            // request was already aborted or advanced. Treat it as stale.
            if !pending_versionchange.remove(&from_id) {
                return;
            }
            ...
            let Some(version) = *version else {
                // The open request may have been pruned while a version-change
                // callback was in flight. Treat this as a stale notification;
                // the request's existing error/close path owns cleanup.
                return;
            };
    ...
    fn open_database(&mut self, key: IndexedDBDescription) {
        ...
        let Some(version) = *version else {
            // A stale open request can be observed after backend cleanup.
            // Do not take down the IndexedDB manager for this notification.
            return;
        };
    ...
    fn close_database(&mut self, origin: ImmutableOrigin, id: Uuid, name: String) {
        ...
        if can_upgrade {
            // Step 10.6: Run upgrade a database using connection, version and request.
            let Some(version) = version else {
                // A stale close/upgrade notification can race with queue
                // pruning.  Treat it as an aborted upgrade instead of
                // panicking the IndexedDB manager thread.
                return;
            };
            self.upgrade_database(key.clone(), version);
